...
<div class="container mx-auto">
    ...
    <form wire:submit.prevent="submit">
        ...
        <div class="mb-3 row d-flex align-items-center">
            <label for="dia" class="col-sm-2 col-form-label">Dia del evento</label>
            <div class="col-sm-10">
        ...
        <div class="mb-3 row d-flex align-items-center">
            <label for="id_evento" class="col-sm-2 col-form-label">Evento </label>
            <div class="col-sm-10">
                <select class="form-control" name="id_evento" required id="id_evento" wire:model="id_evento">
                    <option value="">Eventos</option>
                    @foreach ($eventos as $evento)

                        <option value="{{ $evento->id }}">
                            {{ $evento->eventoNombre }}</option>
                    @endforeach
                    <option value="" wire:click.prevetn="crearEvento">Crear Evento</option>
                    @error('id_evento')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </select>
        ...
        <div class="mb-3 row d-flex align-items-center">
            ...
            <div class="col-sm-10">
                <select class="form-control" name="id_monitor" required id="id_monitor"
                    ...
                    @endforeach
                    <option value="" wire:click.prevetn="crearMonitor">Crear Monitor</option>
                    @error('id_monitor')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </select>
        ...
        <div class="mb-3 row d-flex align-items-center">
            <label for="precioMonitor" class="col-sm-2 col-form-label">precioMonitor </label>
            <div class="col-sm-10">
                <input type="number" wire:model="precioMonitor" class="form-control" name="precioMonitor"
                    id="precioMonitor" placeholder="30">
                @error('precioMonitor')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
